...
vector<POINT> quick_sort(vector<POINT> p, int pid) {
    ...
    if (p.size() == 2) {
        ...
        if (!left_temp.empty()) {
            left = quick_sort(left_temp, pid);
        }

        result = left;
        result.push_back(p[pivot]);
        if (!right_temp.empty()) {
            right = quick_sort(right_temp, pid);
...
vector<POINT> sort_points(vector<POINT> points, int numpro, int pid, vector<POINT> max_min) {
    ...
    printf("pid %d \n", pid);
    vector<POINT> sorted_array = quick_sort(array, pid);
    MPI_Type_create_struct(2, lengths, offsets, types, &POINTS_STRUCT);
    MPI_Type_commit(&POINTS_STRUCT);
    if (pid == 0) {
        int len_temp = 0;
